...
□ 他の系（他の元素の組み合わせ）で計算したい場合
  例えば、ポテンシャルを”Fe-Cr-W”用から”Fe-Ni-Cr”用に変更するには以下のようにします。＃以降はコメントになります。”Fe-Cr-W”に関する行を＃を付けてコメントにし、コメントにされていた”Fe-Ni-Cr”の＃を除いて計算するようにします。加えて”# ----- Output setting -----”などでの元素記号は使用するポテンシャルの順番に書き換えてください。
----------(before)----------
# -----(Fe-Ni-Cr) (FCC)
#pair_style      eam/fs
...
□ tutorial_1_Monte_Carlo
  モンテカルロ法を用いて、入力ファイルで指定した温度に対して熱力学的（ボルツマン分布的）に許される原子配置を計算します。LammpsのExampleにある2次元のMCを書き換えて3次元にしてあります（書き換えに誤りがありましたらすみません）。
