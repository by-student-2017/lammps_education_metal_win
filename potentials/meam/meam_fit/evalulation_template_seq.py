...
cif_directory = 'cif_data' # Directory containing CIF files
os.system(f'rm -f {cif_directory}/*.cif {cif_directory}/*.data {cif_directory}/data.in')
os.system(f'cp {cif_directory}/{element}/*.cif {cif_directory}')

# Directory containing JSON files
json_path = 'data.json'
os.system(f'cp {cif_directory}/{element}/{element}_{json_path} {json_path}')
...
for cif_file in os.listdir(cif_directory):
    ...
    if cif_file.endswith('.cif'):
        ...
        add_weight = ((abs(difference_energy) - 0.0215)/0.0215)**2 # 10 kcal/mol = 0.043 eV
