...
def calculate_properties(elements_combination, omp_num_threads, mpi_num_procs, max_retries=200):
    ...
    for scale in np.linspace((1.0-0.12)**(1/3), (1.0+0.12)**(1/3), 25):
        ...
        cohesive_energy = -(atoms.get_total_energy() - isolated_atom_energy1*3 - isolated_atom_energy2) / len(atoms)
...
for i, combination in enumerate(element_combinations):
    ...
    with open(f'{directory}/results_L12_{element1}-{element2}.json', 'a') as jsonfile:
        json.dump(result, jsonfile, indent=4)
        jsonfile.write('\n')

    with open(f'{directory}/results_L12_{element1}-{element2}.csv', 'a', newline='') as csvfile:
