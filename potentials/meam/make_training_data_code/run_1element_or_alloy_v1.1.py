...
def calculate_properties(elements_combination, omp_num_threads, mpi_num_procs, max_retries=20, lattce='', lat='', npoints=25, primitive_flag=1, PBEsol_flag=0, spin_flag=1, D_flag=1, cutoff=520):
    ...
    for scale in np.linspace((1.0-vrange)**(1/3), (1.0+vrange)**(1/3), npoints):
        ...
        if lattce == 'b1' or lattce == 'dia':
            ...
            charges.append([valence_electrons1-charge[0],
                            valence_electrons2-charge[1],
                            valence_electrons2-charge[2],
                            valence_electrons2-charge[3],
                            valence_electrons2-charge[4]])
            mean_elem1_charge = (valence_electrons2-charge[1] + valence_electrons2-charge[2] + valence_electrons2-charge[3] + valence_electrons2-charge[4])/4
            print(f'Error for element 2 at position No.2: {(valence_electrons2-charge[0])/mean_elem1_charge*100-100} [%]')
    ...
    '''
        ...
        with open("error_log.txt", "a") as file:
            file.write(f"Warning during fitting EOS: {w}.: in {lattce}-{element1}-{element2}\n")
    '''
