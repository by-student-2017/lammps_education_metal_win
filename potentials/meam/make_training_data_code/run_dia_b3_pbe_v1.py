...
def calculate_properties(elements_combination, omp_num_threads, mpi_num_procs, max_retries=200):
    ...
    try:
        ...
        eos.plot('DIA_B3_'+element1+'-'+element2+'_eos.png')
...
for i, combination in enumerate(element_combinations):
    ...
    directory = f'DIA_B3_{element1}-{element2}'
    if not os.path.exists(directory):
        os.makedirs(directory)
    
    # Save intermediate results to a JSON file
    #with open('DIA_B3_'+element1+'-'+element2+'/results_DIA_B3_'+element1+'-'+element2+'.json', 'a') as jsonfile:
    with open(f'{directory}/results_DIA_B3_{element1}-{element2}.json', 'a') as jsonfile:
        json.dump(result, jsonfile, indent=4)
        jsonfile.write('\n')
    
    # Save intermediate results to a CSV file
    #with open('DIA_B3_'+element1+'-'+element2+'/results_DIA_B3_'+element1+'-'+element2+'.csv', 'a', newline='') as csvfile:
    with open(f'{directory}/results_DIA_B3_{element1}-{element2}.csv', 'a', newline='') as csvfile:
...
print("Calculations are complete and results are saved to results_DIA_B3.json and results_DIA_B3.csv.")
