...
def calculate_properties(elements_combination, omp_num_threads, mpi_num_procs, max_retries=200):
    ...
    # Create the BCC B2 (CsCl-type) structure
    ...
    calc = Espresso(pseudopotentials=pseudopotentials_dict, input_data=input_data, kpts=(10, 10, 10), omp_num_threads=omp_num_threads, mpi_num_procs=mpi_num_procs)
    ...
    try:
        ...
        eos.plot('BCC_B2_'+element1+'-'+element2+'_eos.png')
...
for i, combination in enumerate(element_combinations):
    ...
    directory = f'BCC_B2_{element1}-{element2}'
    if not os.path.exists(directory):
        os.makedirs(directory)
    
    # Save intermediate results to a JSON file
    #with open('BCC_B2_'+element1+'-'+element2+'/results_BCC_B2_'+element1+'-'+element2+'.json', 'a') as jsonfile:
    with open(f'{directory}/results_BCC_B2_{element1}-{element2}.json', 'a') as jsonfile:
        json.dump(result, jsonfile, indent=4)
        jsonfile.write('\n')
    
    # Save intermediate results to a CSV file
    #with open('BCC_B2_'+element1+'-'+element2+'/results_BCC_B2_'+element1+'-'+element2+'.csv', 'a', newline='') as csvfile:
    with open(f'{directory}/results_BCC_B2_{element1}-{element2}.csv', 'a', newline='') as csvfile:
...
print("Calculations are complete and results are saved to results_BCC_B2.json and results_BCC_B2.csv.")
