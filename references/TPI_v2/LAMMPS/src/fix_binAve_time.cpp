...
------------------------------------------------------------------------- */
/* ----------------------------------------------------------------------
This is a modified version of fix ave/time which uses one or more global
values as inputs every few timesteps, and average them over longer 
timescales. See the README file the LAMMPS script directory. 
...
FixBinAveTime::FixBinAveTime(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 9) error->all(FLERR,"Illegal fix binAve/time command");
  ...
  nlayers = force->inumeric(FLERR,arg[6]);
  ncols = force->inumeric(FLERR,arg[7]);
    ...
    if (strncmp(arg[iarg],"c_",2) == 0 ||
      ...
      if (ptr) {
        if (suffix[strlen(suffix)-1] != ']')
          error->all(FLERR,"Illegal fix binAve/time command");
      ...
      if (mode == VECTOR && which[nvalues] == COMPUTE &&
        ...
        if (icompute < 0)
          error->all(FLERR,"Compute ID for fix binAve/time does not exist");
        ...
        if (ifix < 0)
          error->all(FLERR,"Fix ID for fix binAve/time does not exist");
  ...
  for (int i = 0; i < noff; i++) {
    if (offlist[i] < 1 || offlist[i] > nvalues)
      error->all(FLERR,"Invalid fix binAve/time off column");
  ...
  for (int i = 0; i < nvalues; i++) {
    if (which[i] == COMPUTE && mode == SCALAR) {
      ...
      if (argindex[i] == 0 && modify->compute[icompute]->scalar_flag == 0)
        error->all(FLERR,"Fix binAve/time compute does not calculate a scalar");
      if (argindex[i] && modify->compute[icompute]->vector_flag == 0)
        error->all(FLERR,"Fix binAve/time compute does not calculate a vector");
      if (argindex[i] && argindex[i] > modify->compute[icompute]->size_vector)
        error->all(FLERR,
                   "Fix binAve/time compute vector is accessed out-of-range");
      ...
      if (argindex[i] && modify->compute[icompute]->array_flag == 0)
        error->all(FLERR,"Fix binAve/time compute does not calculate an array");
      if (argindex[i] &&
          argindex[i] > modify->compute[icompute]->size_array_cols)
        error->all(FLERR,"Fix binAve/time compute array is accessed out-of-range");
      ...
      if (argindex[i] == 0 && modify->fix[ifix]->scalar_flag == 0)
        error->all(FLERR,"Fix binAve/time fix does not calculate a scalar");
      if (argindex[i] && modify->fix[ifix]->vector_flag == 0)
        error->all(FLERR,"Fix binAve/time fix does not calculate a vector");
      if (argindex[i] && argindex[i] > modify->fix[ifix]->size_vector)
        error->all(FLERR,"Fix binAve/time fix vector is accessed out-of-range");
      if (nevery % modify->fix[ifix]->global_freq)
        error->all(FLERR,
                   "Fix for fix binAve/time not computed at compatible time");
      ...
      if (argindex[i] && modify->fix[ifix]->array_flag == 0)
        error->all(FLERR,"Fix binAve/time fix does not calculate an array");
      if (argindex[i] && argindex[i] > modify->fix[ifix]->size_array_cols)
        error->all(FLERR,"Fix binAve/time fix array is accessed out-of-range");
      if (nevery % modify->fix[ifix]->global_freq)
        error->all(FLERR,
                   "Fix for fix binAve/time not computed at compatible time");
      ...
      if (ivariable < 0)
        error->all(FLERR,"Variable name for fix binAve/time does not exist");
      if (input->variable->equalstyle(ivariable) == 0)
        error->all(FLERR,"Fix binAve/time variable is not equal-style variable");
      if (mode == VECTOR)
        error->all(FLERR,"Fix binAve/time cannot use variable with vector mode");
    }
  }
  ...
  if (mode == VECTOR) {
    ...
    for (int i = 0; i < nvalues; i++) {
      ...
      if (i == 0) nrows = length;
        ...
        error->all(FLERR,"Fix binAve/time columns are inconsistent lengths");
    }
  ...
  if (fp && me == 0) {
    ...
    if (title2) fprintf(fp,"%s\n",title2);
      ...
      fprintf(fp,"# TimeStep");
      for (int i = 0; i < nvalues; i++) fprintf(fp," %s",arg[8+i]);
      fprintf(fp,"\n");
  ...
  if (mode == SCALAR) {
    ...
    if (ave == WINDOW)
      memory->create(vector_list,nwindow,nvalues,"binAve/time:vector_list");
  } else {
    memory->create(array,nrows,nvalues,"binAve/time:array");
    memory->create(array_total,nrows,nvalues,"binAve/time:array_total");
    if (ave == WINDOW)
      memory->create(array_list,nwindow,nrows,nvalues,"binAve/time:array_list");
  }
  ...
  if (mode == SCALAR) {
    ...
    if (nvalues == 1) {
      ...
      for (int i = 0; i < nvalues; i++) {
        ...
        if (value == -1)
          error->all(FLERR,"Fix binAve/time cannot set output array "
...
void FixBinAveTime::setup(int vflag)
{
  end_of_step();
}
...
void FixBinAveTime::end_of_step()
{
...
void FixBinAveTime::invoke_scalar(bigint ntimestep)
{
  int i,m;
  double scalar;
...
void FixBinAveTime::invoke_vector(bigint ntimestep)
{
  int i,j,m;
  ...
  if (fp && me == 0) {
    ...
    */
    for (i = 0; i < nlayers; i++) {
      fprintf(fp,"%d",i+1);
      int tmp = (nrows/nlayers)*i;
      for (j = 0; j < nvalues; j++) {
        fprintf(fp,"\t%f",array_total[tmp][j]/norm);
	if (ncols > 1) for (m = 1; m < ncols; m++) fprintf(fp,"\t%e", array_total[tmp+m][j]/norm);
      }
      fprintf(fp,"\n");
    }
    fprintf(fp,"\n");
...
double FixBinAveTime::compute_scalar()
{
...
double FixBinAveTime::compute_vector(int i)
{
  if (norm) {
...
double FixBinAveTime::compute_array(int i, int j)
{
...
void FixBinAveTime::options(int narg, char **arg)
{
  // option defaults
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"file") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix binAve/time command");
      if (me == 0) {
        fp = fopen(arg[iarg+1],"w");
        if (fp == NULL) {
          char str[128];
          sprintf(str,"Cannot open fix binAve/time file %s",arg[iarg+1]);
          error->one(FLERR,str);
        }
      }
      iarg += 2;
      ...
      else error->all(FLERR,"Illegal fix binAve/time command");
      if (ave == WINDOW) {
        if (iarg+3 > narg) error->all(FLERR,"Illegal fix binAve/time command");
        nwindow = force->inumeric(FLERR,arg[iarg+2]);
        if (nwindow <= 0) error->all(FLERR,"Illegal fix binAve/time command");
      }
      iarg += 2;
      if (ave == WINDOW) iarg++;
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix binAve/time command");
      memory->grow(offlist,noff+1,"binAve/time:offlist");
...
void FixBinAveTime::allocate_values(int n)
{
  memory->grow(which,n,"binAve/time:which");
  memory->grow(argindex,n,"binAve/time:argindex");
  memory->grow(value2index,n,"binAve/time:value2index");
  memory->grow(offcol,n,"binAve/time:offcol");
  ids = (char **) memory->srealloc(ids,n*sizeof(char *),"binAve/time:ids");
}
...
bigint FixBinAveTime::nextvalid()
{
...
void FixBinAveTime::reset_timestep(bigint ntimestep)
{
  if (ntimestep > nvalid) error->all(FLERR,"Fix binAve/time missed timestep");
}
