...
r_20 = -20:40/1000:20;
R = 16;
cosSub_CM = (-R<r_20).*(r_20<R).*(0.5*(cos(pi/R.*r_20)+1)).^2 * -16;
substrate = load(['substrate.csv']);
CM_REBO = load('CM_REBO.csv');
CM_SW = load('CM_SW.csv');
load MS_REBO.mat
MS_REBO = data';
load MS_SW.mat
MS_SW = data';
...
% MD
plot(MS_REBO(1,:),MS_REBO(2,:),'--','LineWidth',2,'Color',dark_purple)
plot(MS_SW(1,:),MS_SW(2,:),'--','LineWidth',2,'Color',light_blue) 
% Substrate
plot(r_20,substrate,'k-.','LineWidth',2)

% patch([r_20 fliplr(r_20)], [cosSub_CM  fliplr(-20.*ones(1,1001))], 'g'); 
a = area(r_20,cosSub_CM,-17);
a.FaceAlpha = 0.2;
a.FaceColor = 'k';
% add zoom box:
