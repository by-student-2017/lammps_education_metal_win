/* ----------------------------------------------------------------------
   LAMMPS - Large-scale Atomic/Slecular Massively Parallel Simulator
...
FixAddForceFieldS::FixAddForceFieldS(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
...
FixAddForceFieldS::~FixAddForceFieldS()
{
  /*delete [] xstr;
  delete [] ystr;
  delete [] zstr;
...
void FixAddForceFieldS::setup(int vflag)
{
...
void FixAddForceFieldS::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixAddForceFieldS::post_force(int vflag)
{
  double **x = atom->x;
  double **f = atom->f;
  int *mask = atom->mask;
...
/* Cos shaped trench case (07/26/21) 
!!! Here, r is replaced with x due to convenience for code change */
  radius = 300.0; // the radius of hole =  10 nm
  dr = 0.1;
  dz = 0.1;
  double xclist = radius;
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      ...
      xc = xclist; /* X position of the center of k_th hole*/
      R0 = (atomx-xc);   /* x distance from the xc (for cos shaped trench case)*/      
      /* Flag for */
      double dir_flag = 1.0;
      if (R0 < 0.0) {
	R0 = R0*(-1.0);
	dir_flag = -1.0;} /* Opposite direction */
      z0 = atomz;
      ...
      f00 = U_S[Ri][zi];f01 = U_S[Ri][zi+1];f10 = U_S[Ri+1][zi];f11 = U_S[Ri+1][zi+1];

      //fprintf(screen,"f00 %f f01: %f f10: %f f11: %f\n",f00,f01,f10,f11);

      fr00 = (U_S[Ri+1][zi]-U_S[Ri-1][zi])/2;
      fr10 = (U_S[Ri+2][zi]-U_S[Ri][zi])/2;
      fr01 = (U_S[Ri+1][zi+1]-U_S[Ri-1][zi+1])/2;
      fr11 = (U_S[Ri+2][zi+1]-U_S[Ri][zi+1])/2;

      fz00 = (U_S[Ri][zi+1]-U_S[Ri][zi-1])/2;
      fz10 = (U_S[Ri+1][zi+1]-U_S[Ri+1][zi-1])/2;
      fz01 = (U_S[Ri][zi+2]-U_S[Ri][zi])/2;
      fz11 = (U_S[Ri+1][zi+2]-U_S[Ri+1][zi])/2;

      frz00 = (U_S[Ri+1][zi+1]-U_S[Ri+1][zi-1]-U_S[Ri-1][zi+1]+U_S[Ri-1][zi-1])/2/2;
      frz10 = (U_S[Ri+2][zi+1]-U_S[Ri+2][zi-1]-U_S[Ri][zi+1]+U_S[Ri][zi-1])/2/2;
      frz01 = (U_S[Ri+1][zi+2]-U_S[Ri+1][zi]-U_S[Ri-1][zi+2]+U_S[Ri-1][zi])/2/2;
      frz11 = (U_S[Ri+2][zi+2]-U_S[Ri+2][zi]-U_S[Ri][zi+2]+U_S[Ri][zi])/2/2;
     ...
     // fprintf(screen,"Urz  %17.16f fr: %17.16f fz: %17.16f nuforce: %17.16f atomz: %17.16f R0: %17.16f \n",Urz,-fr/0.1,-fz/0.1,atomz,R0);
...
void FixAddForceFieldS::post_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixAddForceFieldS::min_post_force(int vflag)
{
  post_force(vflag);
}
...
double FixAddForceFieldS::compute_scalar()
{
...
double FixAddForceFieldS::compute_vector(int n)
{
  // only sum across procs one time
...
double FixAddForceFieldS::memory_usage()
{
  double bytes = 0.0;
